...
This is an extension of `cartens` to work with different sets of states for bra and ket quanta,
for example, representing different electronic states
"""
...
class CartTensor2(MKTensor2):
    def __init__(
        ...
        if mol_tens is None:
            print(
                f"WARNING: CartTensor2 matrix elements are not tested for operator '{tens_name}'"
            )
            if vib:
                nvib1 = np.max(
                    [
